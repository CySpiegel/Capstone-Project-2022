...
class GeneticTree:
	...
	'''
	@classmethod
	def declarePrimitive(cls, roles, output_type, input_types):
		...
		def addPrimitive(func):
			for role in roles:
				...
				print(f"importing primitive '{func.__name__}' of type {output_type} for role '{role}'")
			return func
		return addPrimitive
	...
	def initialize(self, depth = 1, hardLimit = 0, grow = False, leafProb = 0.5, full = False):
		if grow:
